...
To check whether the solver's mathematical predictions could survive
contact with the physical world, I built a quick $\sim$60~cm subscale chuck
glider out of cardboard. It featured the $p=4$ squircle ring shape with an
$h/b = 0.5$ height-to-span ratio and a $\sim$15~cm wing stagger, held
together by a bamboo skewer spine and balanced with a lump of modeling clay
in the nose. The vortex-lattice model predicted that the aerodynamic
balance point---the neutral point---should sit between 7.5 and 8~cm behind
the leading edge of the bottom wing, so I marked that spot directly on the
fuselage. Once tossed and trimmed for a smooth, steady glide, the real-world
balance point lined up right on the mark, with the glider routinely covering
4.0 to 4.5 meters from a modest 1.2-meter release height for an estimated
glide ratio ($L/D$) of roughly 3.3 to 3.8. Naturally, a quick hand launch
comes with obvious caveats: the initial throw inflates the apparent
performance, flat-plate cardboard airfoils operate at a low Reynolds number
where viscous drag dominates, and a single low-fidelity test is hardly a
complete validation campaign. Even so, the fundamental prediction held
up---the calculated neutral point landed dead-on when translated to a real,
physical ring wing.

\section{Limitations}

While these results are promising, several key limitations frame the scope
of this study. First, the core analysis is purely inviscid, meaning it
computes only induced drag and ignores skin friction and profile drag, so
total drag on a real ring will inevitably be higher. The model also assumes
an ideal, fully optimized lift distribution acting on a rigid wake, and it
evaluates an untrimmed configuration without accounting for static
stability, except through the empirical adjustments made on the subscale
glider. Furthermore, beyond that simple cardboard flight test, the
predictions have not yet been validated in real viscous air, making a
wind-tunnel campaign or an instrumented flight test the clear next step.
Finally, while the performance relation $e = 1 + h/b$ is supported by strong
numerical evidence, it remains a fitted observation rather than a rigorous
analytical proof. Crucially, however, the model also neglects structural
joint and corner interference losses---a simplification that actually biases
the math against the squircle, suggesting that real-world aerodynamics may
favor the smooth ring over a traditional box wing even more than predicted.
